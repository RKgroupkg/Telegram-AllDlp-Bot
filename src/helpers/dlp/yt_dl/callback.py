...
async def handle_youtube_link(client: Client, message: Message) -> None:
    ...
    if (
        ...
        and active_downloads[user_id]["expiry"] > time.time()
    ):
                ...
                [
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=f"ytcancel_{user_id}",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "+ Add to queue", callback_data=f"ytqueue_{video_id}"
                        )
                    ],
                ]
            )
    ...
    processing_msg = await message.reply_text(
        "⌕ Processing YouTube link...", quote=True
    )
...
async def handle_youtube_callback(
    ...
    if not cached_data and callback_type not in ["ytcancel", "ytqueue"]:
        ...
        await callback_query.answer(
            "⚠ This selection has expired. Please try again.", show_alert=True
        )
        try:
            await message.edit_text(
                "⚠ This selection has expired. Please request the YouTube link again."
            )
    ...
    try:
        if callback_type == "ytcancel":
            ...
            if user_id_to_cancel in active_downloads:
                ...
                if user_id in download_queue and download_queue[user_id]:
                    ...
                    await callback_query.answer(
                        "۞ Starting next download from queue..."
                    )
        ...
        # Modify the first queue implementation to match the second
            ...
            if not info:
                await callback_query.answer(
                    "⚠ Video information not available anymore.", show_alert=True
                )
                return
            ...
            markup = create_format_selection_markup(
                formats, video_id=video_id, page=page
            )
            ...
            if not info:
                await callback_query.answer(
                    "⚠ Video information not available", show_alert=True
                )
                return
            ...
            await start_download(
                client, callback_query, message, video_id, format_id, info, format_id
            )
            ...
            await start_download(
                client, callback_query, message, video_id, "flac", info, "flac"
            )
            ...
            if (
                ...
                queue_markup = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "☩ Add to queue",
                                callback_data=f"ytqueueformat_{queue_data}",
                            )
                        ]
                    ]
                )
                ...
                await callback_query.answer(
                    "⚠ You already have an active download in progress", show_alert=True
                )
                try:
            ...
            if not selected_format:
                await callback_query.answer(
                    "⚠ Selected format not available.", show_alert=True
                )
                return

            # For large files, ask for confirmation
            file_size = selected_format.get(
            ...
            if file_size > 100 * 1024 * 1024:  # Over 100MB
                confirm_markup = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "✔ Download anyway",
                                callback_data=f"ytdlconfirm_{callback_id}",
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "✖  Cancel", callback_data=f"ytcancel_{user_id}"
                            )
                        ],
                    ]
                )
                ...
                await callback_query.answer(
                    "⁂ Large file detected, confirmation needed"
                )
                return
...
async def start_download(
    ...
    ):  # If its a dic which implys that its not flac or ytbestVideo
    ...
    cancel_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✖  Cancel Download", callback_data=f"ytcancel_{user_id}"
                )
            ]
        ]
    )
        ...
        if d["status"] == "downloading":
            ...
            if active_downloads[user_id]["last_progress"] == downloaded_bytes:
                if active_downloads[user_id]["stalled_since"] is None:
                    ...
                    current_time - active_downloads[user_id]["stalled_since"]
                    > STALLED_DOWNLOAD_TIMEOUT
                ):
            ...
            if current_time - last_update_time >= PROGRESS_UPDATE_INTERVAL:
                ...
                progress_text = await format_progress(
                    downloaded_bytes, total_bytes, start_time
                )
    ...
    try:
            ...
            try:
                ...
                download_task = asyncio.create_task(
                    download_youtube_video(
                        video_id, format_id, progress_hook, **options
                    )  # passdown extra formats
                )
                ...
                if attempt < MAX_RETRIES - 1:
                    await message.edit_text(
                        f"⚠ Download error, retrying ({attempt+1}/{MAX_RETRIES})..."
                    )
                    await asyncio.sleep(2)
                else:
                    raise e
        ...
        await message.edit_text(
            f"↥ Uploading __[{format_size(int(filesize))}]__\n\n  __{title}__..."
        )
            ...
            if is_audio:
                ...
                await client.send_video(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton("♙ Open Youtube ♙", url=Url)]]
                    ),
                )
        except Exception as e:
            error_trace = traceback.format_exc()
...
async def process_next_in_queue(client, user_id, message):
    ...
    if user_id in download_queue and download_queue[user_id]:
        ...
        if isinstance(next_item, tuple):
            ...
            if not info:
                await message.edit_text(
                    "✖ Queued video information expired. Please try again."
                )
                await process_next_in_queue(client, user_id, message)
                return
