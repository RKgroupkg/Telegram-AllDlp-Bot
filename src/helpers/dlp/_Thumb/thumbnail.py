...
class ThumbnailManager:
    ...
    async def _download_thumbnail(self, thumbnail_url: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            for attempt in range(retry_count):
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(thumbnail_url, timeout=10, 
                                               raise_for_status=True) as response:
                            content = await response.read()
                            
                            # Validate content
                            if not content or len(content) < 100:
                                logger.warning(f"Downloaded thumbnail too small: {len(content)} bytes")
                            ...
                            if os.path.exists(thumbnail_filename) and os.path.getsize(thumbnail_filename) > 0:
                                logger.info(f"Successfully downloaded thumbnail to {thumbnail_filename}")
            ...
            logger.error(f"Unexpected error during thumbnail download: {str(e)}", exc_info=True)
...
async def download_and_verify_thumbnail(thumbnail_url: Optional[str]) -> Tuple[bool, Optional[str]]:
    """
