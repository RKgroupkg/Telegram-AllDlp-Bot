...
async def send_music_results(
    ...
    try:
        ...
        if page > 0:
            nav_buttons.append(
                InlineKeyboardButton("◄ Previous", callback_data=f"music_page:{page-1}")
            )

        nav_buttons.append(
            InlineKeyboardButton(
                f"Page {page + 1}/{total_pages}", callback_data="page_info"
            )
        )

        if end_idx < len(results):
            nav_buttons.append(
                InlineKeyboardButton("Next ►", callback_data=f"music_page:{page+1}")
            )
        ...
        keyboard.append(
            [
                InlineKeyboardButton("+ More", callback_data="music_load_more"),
                InlineKeyboardButton("✖ Close", callback_data="music_cancel"),
            ]
        )
        ...
        if isinstance(message, Message):
            await message.edit_text(
                full_result_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML
            )
...
@bot.on_callback_query(
    filters.regex("^music_select:") & is_download_callback_rate_limited
)
async def music_select_handler(_, query: CallbackQuery):
    ...
    try:
        ...
        selected_track = next(
            (track for track in results if track.id == video_id), None
        )
