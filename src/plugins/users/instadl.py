# File: src/plugins/users/instadl.py
...
async def instagram_downloader_handler(client: Client, message: Message) -> None:
    ...
    LOGGER(__name__).info(
        f"Instagram link detected from user {user_id} in chat {chat_id}"
    )
    ...
    processing_msg = await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("Open Instagram", url=instagram_urls[0])
                ]  # Take the first URL
            ]
        ),
    )
    ...
    try:
        ...
        if message.chat.type == enums.ChatType.PRIVATE:
            ...
            try:
                ...
                LOGGER(__name__).error(
                    f"An error occurred while getting my chat_member error: {e}"
                )
        ...
        if len(medias) == 1:
            ...
            if len(medias) > 1:
                ...
                for i in range(
                    0, len(medias), 10
                ):  # Changed 1 to 0 to include the first item
                    media_group = []
                    batch = medias[i : i + 10]
                        ...
                        try:
                            ...
                            for item in batch:
                                try:
                                    ...
                                    if item.get("type") == "image":
                                        await message.reply_photo(
                                            photo=media_url, quote=True
                                        )
                                    else:
                                        await message.reply_video(
                                            video=media_url, quote=True
                                        )
                                except Exception as e_inner:
                                    LOGGER(__name__).error(
                                        f"Error sending individual media: {e_inner}"
                                    )
            ...
            if bot_member:  # if it's None then likely it isn't in a group
                if (
                    ...
                    if privileges.can_delete_messages:
                        if (
                            only_instagram_link
                        ):  # Dont remove if has msg not just a link
                            await message_viaUser.delete()
                else:
                    LOGGER(__name__).debug("Bot doesn't have privilege to delete.")
        except Exception as e:
            LOGGER(__name__).error(
                f"An error occurred while trying to delete msg of user in grp/spgrp error: {e}"
            )
...
@Client.on_message(
    filters.command(["instagram", "insta", "igdl"]) & is_download_rate_limited
)
@catch_errors
async def instagram_command_handler(client: Client, message: Message) -> None:
    ...
    processing_msg = await message.reply_text(
        "📥 Processing Instagram link...", quote=True
    )
    ...
    try:
        ...
        if not media_data:
            await processing_msg.edit_text(
                "✖ Failed to extract media from Instagram link."
            )
            return
        ...
        if len(medias) == 1:
            ...
            if media_item.get("type") == "image":
                await message.reply_photo(photo=media_url, caption=caption, quote=True)
            else:  # video or other type
                await message.reply_video(video=media_url, caption=caption, quote=True)
        else:
            # If more than one media, send the rest (up to 10 items per group)
            if len(medias) > 1:
                # Maximum 10 media items per group in Telegram
            ...
            [
                [
                    InlineKeyboardButton(
                        "Try Inline Mode",
                        switch_inline_query_current_chat="https://instagram.com/",
                    )
                ]
            ]
        ),
...
async def instagram_inline_handler(client: Client, inline_query: InlineQuery) -> None:
    ...
    try:
        ...
        if not media_data or not media_data.get("medias"):
            ...
            LOGGER(__name__).info(
                f"Successfully extracted {len(media_data.get('medias', []))} media items"
            )
            ...
            for i, media_item in enumerate(medias):
                ...
                try:
                    if media_type == "image":
                        ...
                        thumb_url = media_item.get(
                            "thumbnail",
                            "https://static.poder360.com.br/2021/12/instagram-logo.jpg",
                        )
                    ...
                    LOGGER(__name__).error(
                        f"Error creating result for media item {i+1}: {item_error}"
                    )
                    inline_results.append(
                        InlineQueryResultArticle(
        ...
        LOGGER(__name__).info(
            f"Answering inline query with {len(inline_results)} results"
        )
        await inline_query.answer(
            results=inline_results, cache_time=3600  # Cache for 1 hour
        )
