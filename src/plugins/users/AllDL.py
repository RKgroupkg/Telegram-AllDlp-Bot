...
class DownloadManager:
    ...
    def clear_progress(self, chat_id: int, msg_id: int) -> None:
        ...
        if (
            chat_id in self.download_progress
            and msg_id in self.download_progress[chat_id]
        ):
...
def download_handler(func):
    ...
    async def wrapper(client, message, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"Error in {func.__name__} for chat {chat_id}: {e}", exc_info=True
            )
            if msg:
                await msg.edit_text(
                        ...
                        [
                            [
                                InlineKeyboardButton(
                                    "✿ Try Again", callback_data="retry_download"
                                )
                            ]
                        ]
                    ),
            ...
            if (
                ...
                try:
                    ...
                    logger.error(
                        f"Error cleaning up file {download_info.file_path}: {e}"
                    )

            # Progress tracking cleanup
            if (
                chat_id in download_manager.download_progress
                and msg
                and msg.id in download_manager.download_progress[chat_id]
            ):
...
@Client.on_callback_query(
    filters.regex(r"^cancel_dl$") & is_download_callback_rate_limited
)
async def handle_cancel_download(client: Client, callback_query: CallbackQuery):
    ...
    if (
        chat_id in download_manager.download_progress
        and message_id in download_manager.download_progress[chat_id]
    ):
        # Mark as cancelled in the progress data
        if download_manager.download_progress[chat_id][message_id].get(
            "cancel_callback"
        ):
            # Call the cancellation callback
            try:
                await download_manager.download_progress[chat_id][message_id][
                    "cancel_callback"
                ]()
                ...
                await callback_query.answer(
                    "⚠ Failed to cancel download", show_alert=True
                )
        else:
            await callback_query.answer(
                "⚠ This download cannot be cancelled", show_alert=True
            )
    else:
...
@Client.on_callback_query(
    filters.regex(r"^retry_download$") & is_download_callback_rate_limited
)
async def handle_retry_download(client: Client, callback_query: CallbackQuery):
    ...
    if original_message:
        ...
        await callback_query.answer(
            "Cannot retry, original message not found", show_alert=True
        )


@Client.on_callback_query(
    filters.regex(r"^queue_info$") & is_download_callback_rate_limited
)
...
@Client.on_message(
    ytdlp_url & allowed_url & filters.incoming & filters.text & is_download_rate_limited
)
...
async def video_handler(client: Client, message: Message):
    ...
    if download_manager.is_chat_active(chat_id):
        ...
        download_manager.register_progress(
            chat_id,
            queued_msg.id,
            {
    ...
    try:
        ...
        while retry_count <= max_retries:
            ...
            try:
                download_info = await download_video_from_link(
                    link, progress_callback=progress_callback, cancel_event=cancel_event
                )
                ...
                logger.error(
                    f"Download error (attempt {retry_count}/{max_retries}): {e}"
                )
                retry_count += 1
        ...
        if not download_info or not download_info.success:
            error_msg = (
                download_info.error if download_info else "Unknown error occurred."
            )
            await msg.edit_text(
        ...
        title = (
            (download_info.title[: Config.TITLE_MAX_LENGTH] + "..")
            if download_info.title
            and len(download_info.title) > Config.TITLE_MAX_LENGTH
            else (download_info.title or "Unknown")
        )
        ...
        if thumbnail:
            ...
            logger.info(
                f"Thumbnail download status: {is_thumbnail_ok}, path: {thumb_path}"
            )
        else:
        ...
        try:
            if file_type == "audio":
                await client.send_audio(
                    ...
                    reply_markup=get_callback_keyboard(
                        link, download_info, processing=False
                    ),
                )
            ...
            logger.info(
                f"Upload completed in {upload_time:.2f} seconds for file {download_info.title}.{ext}"
            )
