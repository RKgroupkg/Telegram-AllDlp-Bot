...
from TelegramBot.config import CATCH_PATH
...
def create_file_list_keyboard(
    ...
    keyboard.append([
        InlineKeyboardButton("♤ Clear All", callback_data=f"{callback_prefix}_clear_all")
    ])
...
async def catch_file_manager(client, message: Message):
    ...
    """
    catch_files = [
        os.path.join(CATCH_PATH, f) for f in os.listdir(CATCH_PATH) 
...
async def catch_callback_handler(client, callback_query):
    ...
    if data.startswith("catch_page_"):
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("♧ Delete", callback_data=f"catch_delete_{file_path}")],
        ...
        deleted_count = sum(1 for file_path in catch_files if os.unlink(file_path) is None)
